...
function writeTechnicalMD(agg) {
  const ts = new Date().toISOString().slice(0, 19).replace('T', ' ');
  const lines = [];
  lines.push(`# Reporte Técnico E2E iOS - ${ts}`);
  lines.push('');
  lines.push('- Resumen');
...
function writeExecutiveMD(agg) {
  const ts = new Date().toISOString().slice(0, 19).replace('T', ' ');
  const successPct = agg.tests
    ? ((agg.passed / agg.tests) * 100).toFixed(2)
    : '0.00';
  const lines = [];
  lines.push(`# Reporte Ejecutivo E2E iOS - ${ts}`);
  lines.push('');
  ...
  lines.push('- Flujo OTP estable en simulador iOS.');
  lines.push('');
...
function main() {
  ...
  const tech = writeTechnicalMD(agg);
  const exec = writeExecutiveMD(agg);
