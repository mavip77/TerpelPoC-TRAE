...
export default function MiBolsillo({navigation}: Props) {
  ...
  const trySend = () => {
    if (montoNum >= saldo) {
      Alert.alert(
        'Saldo insuficiente',
        'El valor a transferir no puede ser superior al saldo del bolsillo',
      );
      return;
    }
    if (!canSend) {
    ...
    Alert.alert(
      ...
      `Se transfirieron $ ${montoNum.toLocaleString(
        'es-CO',
      )} a documento ${docType}-${docNumber}`,
    );
      ...
      <ScrollView
        ...
        <View style={styles.tabsRow}>
          <TouchableOpacity
            style={[
              styles.tabBtn,
              tab === 'recargar' ? styles.tabActive : null,
            ]}
            onPress={() => setTab('recargar')}>
            <Text
              style={[
                styles.tabText,
                tab === 'recargar' ? styles.tabTextActive : null,
              ]}>
              Recargar
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            ...
            <Text
              style={[
                styles.tabText,
                tab === 'transferir' ? styles.tabTextActive : null,
              ]}>
              Transferir
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[
              styles.tabBtn,
              tab === 'movimientos' ? styles.tabActive : null,
            ]}
            onPress={() => setTab('movimientos')}>
            <Text
              style={[
                styles.tabText,
                tab === 'movimientos' ? styles.tabTextActive : null,
              ]}>
              Transacciones
            </Text>
          </TouchableOpacity>
        ...
        {tab === 'transferir' ? (
          <View style={styles.card}>
            ...
            <View style={styles.comboRow}>
              <TouchableOpacity
                ...
                <Text
                  style={[
                    styles.comboText,
                    docType === 'CC' ? styles.comboTextActive : null,
                  ]}>
                  CC
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                ...
                <Text
                  style={[
                    styles.comboText,
                    docType === 'CE' ? styles.comboTextActive : null,
                  ]}>
                  CE
                </Text>
              </TouchableOpacity>
            ...
            <Text
              style={[
                styles.hint,
                docValid ? styles.hintOk : styles.hintError,
              ]}>
              {docValid ? 'Documento válido' : 'Ingresa un documento válido'}
            </Text>
            ...
            <TouchableOpacity
              ...
              <MaterialCommunityIcons
                name="star"
                size={18}
                color={favoritos.length === 0 ? '#9CA3AF' : COLORS.red}
              />
              <Text
                style={[
                  styles.secondaryBtnText,
                  favoritos.length === 0
                    ? styles.secondaryBtnTextDisabled
                    : null,
                ]}>
                Enviar a favoritos
              </Text>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.presetsRow}>
              {['150000', '200000', '350000'].map(v => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.presetBtn,
                    recarga === v ? styles.presetActive : null,
                  ]}
                  onPress={() => setRecarga(v)}>
                  <Text
                    style={[
                      styles.presetText,
                      recarga === v ? styles.presetTextActive : null,
                    ]}>
            ...
            <View style={styles.amountBadge}>
              <Text style={styles.amountBadgeText}>
                $ {recargaNum.toLocaleString('es-CO')}
              </Text>
            </View>

            <TouchableOpacity
              onPress={() => setBeneficiosOpen(o => !o)}
              style={styles.collapseHeader}>
              <Text style={styles.cardTitle}>
                ¿Cómo funciona el porcentaje de beneficios?
              </Text>
              <MaterialCommunityIcons
                name={beneficiosOpen ? 'chevron-up' : 'chevron-down'}
                size={20}
                color={COLORS.mid}
              />
            </TouchableOpacity>
            {beneficiosOpen ? (
              <View style={styles.tableBox}>
                <View style={styles.tableHeader}>
                  <Text style={styles.tableCellHead}>Recarga desde</Text>
                  <Text style={styles.tableCellHead}>Hasta</Text>
                  <Text style={styles.tableCellHead}>% Beneficio</Text>
                </View>
                {[
                  ['20000', '299999', '0.00%'],
                  ['300000', '499999', '0.50%'],
                  ...
                  <View key={`row-${i}`} style={styles.tableRow}>
                    <Text style={styles.tableCell}>
                      $ {Number(r[0]).toLocaleString('es-CO')}
                    </Text>
                    <Text style={styles.tableCell}>
                      $ {Number(r[1]).toLocaleString('es-CO')}
                    </Text>
                    <Text style={styles.tableCell}>{r[2]}</Text>
                  </View>
                ))}
                <Text style={styles.hint}>
                  Dependiendo del valor de tu recarga recibirás un porcentaje de
                  beneficio hasta 1,00%.
                </Text>
              </View>
            ) : null}

            <TouchableOpacity
              disabled={!canRecargar}
              style={[
                styles.primaryBtn,
                !canRecargar ? styles.primaryBtnDisabled : null,
              ]}>
        ...
        <Modal
          ...
          style={styles.modal}
          onBackdropPress={() => setDetailOpen(false)}
          swipeDirection="down"
          onSwipeComplete={() => setDetailOpen(false)}
          backdropOpacity={0.5}
          ...
          <View style={styles.sheet}>
            ...
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>N° de transacción</Text>
              <Text style={styles.detailValue}>70058e…</Text>
            </View>
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Fecha y hora</Text>
              <Text style={styles.detailValue}>
                {selectedTx
                  ? new Date(selectedTx.date).toLocaleString('es-CO')
                  : ''}
              </Text>
            </View>
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Valor transferido</Text>
              <Text style={styles.detailValue}>
                {selectedTx
                  ? `$ ${Math.abs(selectedTx.amount).toLocaleString('es-CO')}`
                  : ''}
              </Text>
            </View>
            <TouchableOpacity style={styles.supportBtn}>
              <MaterialCommunityIcons
                ...
                color={COLORS.red}
              />
              <Text style={styles.supportText}>Chat de soporte</Text>
            </TouchableOpacity>
          </View>
        </Modal>
        ...
        <Modal
          ...
          swipeDirection="down"
          onSwipeComplete={() => setFavListOpen(false)}
          backdropOpacity={0.5}
          animationIn="slideInUp"
          animationOut="slideOutDown">
          <View style={styles.sheet}>
            <View style={styles.sheetHeader}>
              ...
              <TouchableOpacity onPress={() => setFavListOpen(false)}>
                <MaterialCommunityIcons
                  name="close"
                  size={20}
                  color={COLORS.mid}
        ...
        <Modal
          isVisible={favConfirmOpen}
          style={styles.modal}
          onBackdropPress={() => setFavConfirmOpen(false)}
          swipeDirection="down"
          onSwipeComplete={() => setFavConfirmOpen(false)}
          backdropOpacity={0.5}
          animationIn="slideInUp"
          animationOut="slideOutDown">
          <View style={styles.sheet}>
            <View style={styles.sheetHeader}>
              ...
              <TouchableOpacity onPress={() => setFavConfirmOpen(false)}>
                <MaterialCommunityIcons
                  name="close"
                  size={20}
                  color={COLORS.mid}
            ...
            {selectedFav ? (
              <>
                <View style={styles.detailRow}>
                  ...
                  <Text style={styles.detailValue}>{selectedFav.name}</Text>
                </View>
                <View style={styles.detailRow}>
